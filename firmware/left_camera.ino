...
void setup() {
 ...
 // cfg.setJpeg(55);
  ...
  // ---- mDNS ----
  if (MDNS.begin("firecamleft")) {
    Serial.println("mDNS started: http://firecamleft.local");
  }

  // ---- Routes ----
  server.on("/left/cam-lo.jpg",  handleJpgLo);
  server.on("/left/cam-mid.jpg", handleJpgMid);
  server.on("/left/cam-hi.jpg",  handleJpgHi);
  server.on("/left/stream", HTTP_GET, handleMjpegStream);
  ...
  Serial.println("  http://firecamleft.local/left/stream");     // live video MPEG
  Serial.println("----live vieo single frame ------");
  Serial.println("  http://firecamleft.local/left/cam-mid.jpg"); // single Frame
  Serial.println("  http://firecamleft.local/left/cam-mid.jpg");
  Serial.println("  http://firecamleft.local/left/cam-mid.jpg");
}
